    ...
    if (rgb_panel->num_fbs > 0) {
        ...
        for (int i = 0; i < rgb_panel->num_fbs; i++) {
            if (fb_in_psram) {
                ...
                // calloc not only allocates but allo zero's the buffer. We have to make sure this is
                // properly committed to the PSRAM, otherwise all sorts of visual corruption happen.
